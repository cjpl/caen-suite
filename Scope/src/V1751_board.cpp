...
#pragma implementation "V1751_board.h"
#endif
...
V1751Board::V1751Board( int board_index, void (*scope_refresh)( int, bool), void (*update_samples_ready_led)( bool), wxMutex *p_data_mutex, AppSettings *p_app_settings, V17XXBoard::BOARD_VARIANT board_variant, bool is_des_mode): 
...
bool V1751Board::Init( long	vme_handle)
{
	...
	// Prompt even channel disconnect here
	//if( this->m_is_des_mode)
	//	wxMessageBox( wxT("CAENScope is about to perform an ADCs' calibration!\nPlease make sure that even channels' signals are disconnected "), wxT("Warning"), wxOK | wxCENTRE | wxICON_WARNING  );
	...
	// wait for a while after the trigger Sw (to avoid lock with DT5751 on DES mode)
	::wxMilliSleep( 300);
...
bool V1751Board::GetBoardMemory( UINT32 &num_ksamples_per_channel)
{
	...
	// HACK rivedere qui V1751Board::GetBoardMemory
	num_ksamples_per_channel/= 2;
	num_ksamples_per_channel*= 7;		// 10 bit data
	return true;
}
