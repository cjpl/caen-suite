...
int ProgramDigitizer(int handle, WaveDumpConfig_t WDcfg, CAEN_DGTZ_BoardInfo_t BoardInfo)
{
  ...
  if( WDcfg.InterruptNumEvents > 0) {
    /* Interrupt handling */
    if( ret |=
	CAEN_DGTZ_SetInterruptConfig( handle, CAEN_DGTZ_ENABLE,
				      VME_INTERRUPT_LEVEL, VME_INTERRUPT_STATUS_ID,
				      WDcfg.InterruptNumEvents, INTERRUPT_MODE)
	!= CAEN_DGTZ_Success) {
...
void CheckKeyboardCommands(int handle, WaveDumpRun_t *WDrun, WaveDumpConfig_t *WDcfg, CAEN_DGTZ_BoardInfo_t BoardInfo)
{
  ...
  if ((c < '9') && (c >= '0')) {
	...
	(BoardInfo.FamilyCode == CAEN_DGTZ_XX742_FAMILY_CODE)){
      if ( (BoardInfo.FamilyCode == CAEN_DGTZ_XX742_FAMILY_CODE) &&
	   (WDcfg->FastTriggerEnabled == 0) && (ch == 8))
	WDrun->ChannelPlotMask = WDrun->ChannelPlotMask ;
...
int main(int argc, char *argv[])
{
  ...
  if(WDcfg.useCorrections != -1) { // use Corrections Manually
    if (ret = CAENComm_OpenDevice((CAENComm_ConnectionType)WDcfg.LinkType,
				  WDcfg.LinkNum,WDcfg.ConetNode,WDcfg.BaseAddress,&handle)) {
  ...
  ret = CAEN_DGTZ_OpenDigitizer(WDcfg.LinkType, WDcfg.LinkNum, WDcfg.ConetNode,
				WDcfg.BaseAddress, &handle);
  if (ret) {
    ErrCode = ERR_DGZ_OPEN;
    goto QuitProgram;
  }
