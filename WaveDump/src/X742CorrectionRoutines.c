...
void ApplyDataCorrection(uint32_t group, int CorrectionLevelMask,
			 CAEN_DGTZ_DRS4Frequency_t frequency, CAEN_DGTZ_X742_GROUP_t *data,
  ...
  for (j=0;j<8+trg;j++) {
    ...
    for(i=1; i<1024; i++)  {
      ...
      vcorr =( ((float)(data->DataChannel[j][k] - data->DataChannel[j][k-1]) /
		(Time[k]-Time[k-1]))*((i*Tsamp)-Time[k-1]) );
